...
auto start=std::chrono::steady_clock::now();double maxseconds=60;bool stopped=false;
...
constexpr int prime=257;int inverse_[prime];
...
int main(int argc,char**argv){
 if(argc!=4){std::cerr<<"usage: enumerate_general SYSTEM MAX_MULTIPLICITY MAX_SECONDS\n";return 2;}
 std::ifstream in(argv[1]);int ne,np;in>>rank_>>pairs_>>nv>>ne>>np;if(!in)return 2;
 M=std::stoi(argv[2]);maxseconds=std::stod(argv[3]);
 if(rank_<4||rank_>8||nv<1||nv>128||ne<1||ne>2048||pairs_<0||pairs_>(rank_-1)/2||M<1||M>15||maxseconds<=0)return 2;
 eqs.resize(ne);inc.resize(nv);counts.resize(M+1);
 for(int i=0;i<ne;i++){auto&e=eqs[i];int n;in>>n;e.ts.resize(n);for(auto&t:e.ts){in>>t.a>>t.b>>t.c;if(t.a>=0)e.vs.push_back(t.a);if(t.b>=0)e.vs.push_back(t.b);}std::sort(e.vs.begin(),e.vs.end());e.vs.erase(std::unique(e.vs.begin(),e.vs.end()),e.vs.end());for(int v:e.vs)inc[v].push_back(i);}
 ...
 if(!in){std::cerr<<"Malformed system file\n";return 2;}
 dom.assign(nv,(1u<<(M+1))-1);search();
 double sec=std::chrono::duration<double>(std::chrono::steady_clock::now()-start).count();
 std::cerr<<"rank "<<rank_<<" pairs "<<pairs_<<" bound "<<M<<" complete "<<(!stopped)<<" nodes "<<nodes<<" classes "<<sols<<" seconds "<<sec<<" counts";for(int m=1;m<=M;m++)std::cerr<<' '<<counts[m];std::cerr<<'\n';if(!std::cout){std::cerr<<"Output write failed\n";return 2;}return stopped?3:0;
}
