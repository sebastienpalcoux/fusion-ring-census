// C++17, ranks 4--7, multiplicity 1--30. No source database is read.
...
bool gauss(std::vector<int>&q,std::vector<char>&inq){
 ...
 int A[200][81];int rows=0;
 for(auto&e:eqs){
  int row[81]={};bool nonlinear=false;
  for(auto&t:e.ts){
  ...
  if(nonzero){for(int j=0;j<=n;j++)A[rows][j]=row[j];rows++;}
 }
 int h=0;int piv[80];
...
CompletionSolver(const char *path,int bound,double seconds,std::chrono::steady_clock::time_point began,int unit_weight=1){
 ...
 if(!in||nv>80||ne>200||bound<1||bound>30)throw std::runtime_error("invalid system");
