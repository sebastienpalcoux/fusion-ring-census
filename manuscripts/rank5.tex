...
\vspace{0.35cm}
{\sffamily\Large A reproducible census through multiplicity 18\par}
...
\begin{abstract}
We explain the exact enumeration of all based fusion rings of rank 5 through multiplicity 18, comprising 28,493 isomorphism classes. The account separates the mathematical coverage argument, the completed search, and independent verification of the emitted tensors. All duality types are included, and no categorifiability filter is applied. Code, machine-readable counts, full multiplication tensors and provenance records accompany the text.
...
The released bound is $M=18$ and the cumulative number of classes is $C_{5}(18)=28,493$. Every duality type is included. The accompanying full tensor file has one record per based-isomorphism class.
...
\begin{lstlisting}
python3 code/run_census.py --rank 5 --bound 18 \
  --seconds 1200 --threads 4 --verify --out rerun_rank5
\end{lstlisting}
