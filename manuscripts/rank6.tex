...
\vspace{0.35cm}
{\sffamily\Large A reproducible census through multiplicity 7\par}
...
\begin{abstract}
We explain the exact enumeration of all based fusion rings of rank 6 through multiplicity 7, comprising 9,613 isomorphism classes. The account separates the mathematical coverage argument, the completed search, and independent verification of the emitted tensors. All duality types are included, and no categorifiability filter is applied. Code, machine-readable counts, full multiplication tensors and provenance records accompany the text.
...
The released bound is $M=7$ and the cumulative number of classes is $C_{6}(7)=9,613$. Every duality type is included. The accompanying full tensor file has one record per based-isomorphism class.
...
The data and their checksums are recorded in \path{results/census.json}. The corresponding completed-run evidence and independent audit are under \path{verification/rank6/}.
...
\begin{lstlisting}
python3 scripts/run_laptop.py --rank 6 --bound 7
\end{lstlisting}
\end{minipage}\par
