...
\vspace{0.35cm}
{\sffamily\Large A reproducible census through multiplicity 8\par}
...
\begin{abstract}
We explain the exact enumeration of all based fusion rings of rank 6 through multiplicity 8, comprising 16,612 isomorphism classes. The account separates the mathematical coverage argument, the completed search, and independent verification of the emitted tensors. All duality types are included, and no categorifiability filter is applied. Code, machine-readable counts, full multiplication tensors and provenance records accompany the text.
...
The released bound is $M=8$ and the cumulative number of classes is $C_{6}(8)=16,612$. Every duality type is included. The accompanying full tensor file has one record per based-isomorphism class.
