...
\vspace{0.35cm}
{\sffamily\Large A reproducible census through multiplicity 1\par}
...
\begin{abstract}
We explain the exact enumeration of all based fusion rings of rank 8 through multiplicity 1, comprising 96 isomorphism classes. The account separates the mathematical coverage argument, the completed search, and independent verification of the emitted tensors. All duality types are included, and no categorifiability filter is applied. Code, machine-readable counts, full multiplication tensors and provenance records accompany the text.
...
The released bound is $M=1$ and the cumulative number of classes is $C_{8}(1)=96$. Every duality type is included. The accompanying full tensor file has one record per based-isomorphism class.
...
\begin{lstlisting}
python3 code/run_census.py --rank 8 --bound 1 \
  --seconds 4200 --threads "$(nproc)" --verify --out rerun_rank8
\end{lstlisting}
\end{minipage}\par
